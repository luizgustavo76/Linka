...
def new_session():
    ...
    if resultado_username:
        ...
        if senha_descodificada:
            ...
            conn.commit()
            cur.execute("INSERT INTO tokens (username, token, emission_date, expire_date) VALUES(?, ?, ?, ?)", (username, token, str(date), str(expire_date)))
            conn.commit()
            conn.close()
...
def valide():
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT token, username, expire_date FROM tokens WHERE token = ? AND username = ?", (token, username))
    result = cur.fetchone()
    ...
    expire_date = datetime.fromisoformat(result[2])
    ...
    if token == result[0]:
        ...
        cur.execute("INSERT INTO tokens (username, token, emission_date, expire_date) VALUES(?, ?, ?, ?)", (username, token_hex, str(emission_date), str(expire_date)))
        conn.commit()
        conn.close()
